...
def help_command(update: Update, _: CallbackContext) -> None:
    help_text = "Этот бот создан для выполнения практического задания 5 модуля PT Start 2024. Был создан Суровым Александром.\n"
...
def save_phone_numbers(phone_numbers):
    ...
    for number in phone_numbers:
        cur.execute("INSERT INTO phones (phone) VALUES (%s) ON CONFLICT DO NOTHING", (number,))
    conn.commit()
    cur.close()
    conn.close()
...
def save_email_address(email_address):
    ...
    for number in email_address:
        cur.execute("INSERT INTO emails (emails) VALUES (%s) ON CONFLICT DO NOTHING", (number,))
    conn.commit()
    cur.close()
    conn.close()
...
def get_emails(update: Update, context: CallbackContext):
    get_db_data(update, context, "SELECT * FROM emails;")

def get_phone_numbers(update: Update, context: CallbackContext):
    get_db_data(update, context, "SELECT * FROM phones;")
...
def get_repl_logs(update, context):
    command = "cat /var/log/auth.log | grep repl"
